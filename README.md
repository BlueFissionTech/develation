...
- Use `Val::make($value)`, `Val::copy()`, `Val::as($target)`, `snapshot()`, `recall()`, `reset()`, and `if(...)->then(...)->otherwise(...)` when generic values need lifecycle, branching, or change tracking. After `if()` starts a conditional chain, boolean helpers such as `and()`, `or()`, `xor()`, `nor()`, and `not()` mutate the temporary condition only; `then()` / `otherwise()` consume it, and `endif()` discards it explicitly.
