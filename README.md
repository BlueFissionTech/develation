...
- Use `Arr::values($array)` to reindex list values while preserving order. Use `BlueFission\Net\HTTP::pathSegment($segment)` to encode a single URL path segment, and `HTTP::urlScheme()`, `HTTP::urlHost()`, `HTTP::urlPort()`, or `HTTP::urlParts()` for normalized URL component parsing.
