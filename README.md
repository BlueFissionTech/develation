...
- `Arr::toArray($value)` preserves arrays, wraps scalar and generic object values, and converts DevElation `ICollection` implementations through their explicit `contents()` contract.
...
- Use `BlueFission\Data\FileSystem::lines($eol)` for read-only file line values and `FileSystem::entries()` for sorted directory entry values. Missing targets return empty arrays and are not created. Use `FileSystem::fileBasename($path)` and `FileSystem::pathDirectory($path)` for path components, `FileSystem::mkdir($dir, recursive: true, permissions: 0777)` for bounded nested directory creation, and `copy()` / `move()` for package-owned file transfer operations that preserve status and event metadata.
