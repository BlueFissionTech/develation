...
trait StateMachine {
	...
	private function behaviorIsDenied( $behaviorName ) {
		foreach ( $this->_state as $state => $args ) {
			if ( ( Value::is($this->_deniedBehaviors[$state]) && DevArray::count($this->_deniedBehaviors[$state] > 0) ) &&
	...
	private function behaviorIsAllowed( $behaviorName ) {
		foreach ( $this->_state as $state => $args ) {
			if ( (Value::is($this->_allowedBehaviors[$state]) && DevArray::count($this->_allowedBehaviors[$state] > 0) ) &&
