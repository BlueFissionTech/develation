...
class Util {
    ...
    static function emailAdmin($message = '', $subject = '', $from = '', $rcpt = '') {
        $message = (Value::isNotNull($message)) ? $message : "If you have recieved this email, then the admnistrative alert system on your website has been activated with no status message. Please check your log files.\n";
        $subject = (Value::isNotNull($subject)) ? $subject : "Automated Email Alert From Your Site!";
        $from = (Value::isNotNull($from)) ? $from : "admin@" . HTTP::domain();
        $rcpt = (Value::isNotNull($rcpt)) ? $rcpt : "admin@" . HTTP::domain();
    ...
    static function parachute(&$count, $max = '', $redirect = '', $log = false, $alert = false) {
        $max = (Value::isNotNull($max)) ? $max : 400;
        if ($count >= $max) {
            ...
            if (Value::isNotNull($redirect)) HTTP::redirect($redirect, array('msg'=>$status));
    ...
    static function value($var, $filter = FILTER_DEFAULT ) {
		...
		return ( Value::isNotNull($cookie) ) ? $cookie : ( ( Value::isNotNull($post) ) ? $post : $get);
	}
}